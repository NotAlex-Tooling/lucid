 ...
 * Forensic Analysis
 *
 * Manipulation-screening utilities: Error Level Analysis, JPEG
 * quantization-table fingerprinting with quality estimation, and
 * embedded EXIF thumbnail extraction.
 */

export async function computeELA(sourceCanvas, quality = 0.75, amplify = 15) {
    // Re-encodes the canvas as JPEG and returns the amplified per-channel difference
...
export function parseJPEGInternals(arrayBuffer) {
    // Walks JPEG markers and extracts quantization tables, frame info, and the marker sequence
...
export function estimateJPEGQuality(quantTable) {
    // Estimates JPEG quality by comparing a luminance table against the IJG standard
    const STD_LUMA = [
