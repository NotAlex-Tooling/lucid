...
export function applyUnsharpMask(data, width, height, amount, radius = 2, threshold = 0) {
    // Gaussian-style unsharp mask applied where the local difference exceeds the threshold
    if (amount <= 0) return;
...
export function applyAutoWhiteBalance(data) {
    // Gray-world white balance: scales each channel so its mean matches the overall mean
