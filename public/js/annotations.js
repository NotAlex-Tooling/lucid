 ...
 * Annotation Tools
 *
 * Draws annotations over images (shapes, arrows, text, measurements)
 * and blur regions. Blur regions are stored as parameters only and
 * re-rendered from the live pipeline; every action is reported via the
 * onAction callback for the unified undo/redo stack.
 */
...
export class AnnotationManager {
    ...
    applyBlursTo(ctx) {
        // Applies all blur regions to the given context from its current pixels
    ...
    redraw() {
        // Redraws all saved annotations onto the overlay
        this.ctx.clearRect(0, 0, this.canvas.width, this.canvas.height);
