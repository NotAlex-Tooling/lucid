...
class ImageForensicsTool {
    ...
    registerServiceWorker() {
        // Registers the service worker for offline operation
    ...
    restorePrefs() {
        ...
        if (p.magnifier) {
            const m = this.magnifierSettings;
            const setVal = (id, value, text) => {
                const el = document.getElementById(id);
                if (!el) return;
                el.value = value;
                const v = el.parentElement?.querySelector('.value');
                if (v) v.textContent = text ?? String(value);
            };
            const setCheck = (id, val) => { const el = document.getElementById(id); if (el) el.checked = val; };
            setVal('magLevel', m.level, m.level + 'x');
            setVal('magSize', m.size);
            setVal('magEnhance', m.enhancement);
    ...
    loadImageFile(file) {
        // Loads an image File, extracts metadata, and initializes the canvas
    ...
    loadImageFromBlob(blob, sourceLabel = 'Clipboard') {
        // Loads an image from a Blob (paste, clipboard, or URL fetch)
    ...
    animateImageArrival() {
        // Plays a brief reveal animation when the working image changes
        const wrapper = document.querySelector('.canvas-wrapper');
        if (!wrapper) return;
    ...
    syncAnnotationCanvas() {
        // Syncs the annotation and overlay canvas sizes with the main canvas layout
    ...
    recordAnnotationAction(action) {
        // Records annotation and blur actions on the unified history stack
    ...
    async runELA() {
        // Renders an Error Level Analysis view for manipulation screening
        if (!this.originalImage) {
    ...
    applyPerspective(corners) {
        // Warps the marked quadrilateral to an upright rectangle and swaps it in
        const srcCanvas = document.createElement('canvas');
        srcCanvas.width = this.canvas.width;
        srcCanvas.height = this.canvas.height;
    ...
    swapWorkingImage(dataUrl, label, opts = {}) {
        // Replaces the working image as an undoable command
    ...
    renderSplitCompare() {
        // Composites the draggable before/after split view
    ...
    startOcrRegionSelect() {
        // Enters one-shot region selection for OCR
        if (!this.originalImage) {
            showToast('Please load an image first.', 'warning');
            return;
        }
    ...
    async reportImageDataUrl(source, maxDim = 1600) {
        // Renders a source into a size-capped JPEG data URL for the report
    ...
    async exportReport() {
        // Generates a self-contained HTML case report
        if (!this.originalImage) {
            showToast('Please load an image first.', 'warning');
            return;
        }
...
<footer>Generated by Lucid. All analysis was performed locally in the browser; the source image was not transmitted to any server. This report is self-contained and suitable for archiving alongside the original evidence file.</footer>
    ...
    toggleCrop() {
        // Toggles crop mode and applies the crop when a selection exists
        if (!this.transform.cropMode &&
    ...
    pickColor(e) {
        // Picks a color from the image at the click position
    ...
    switchTab(e) {
        // Switches the active sidebar tab and panel
    ...
    async extractEXIF(file, sourceLabel = null) {
        // Wrapper ensuring metadata extraction never fails silently
    ...
    async extractEXIFInner(el, file, sourceLabel = null) {
        // Extracts and displays file metadata, hash, EXIF, JPEG internals, and thumbnail
        let html = '';
    ...
    computeImageStats() {
        // Analyzes the working image and returns percentiles and channel means for presets
        const maxDim = 320;
    ...
    applyPreset(preset) {
        // Applies an enhancement preset as an undoable single pipeline pass
        if (!this.originalImage) return;
    ...
    async runOCR(sourceCanvas = null) {
        // Runs Tesseract.js OCR on the image or a supplied region canvas
        if (!this.originalImage) {
            showToast('Please load an image first.', 'warning');
            return;
        }
