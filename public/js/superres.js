 ...
 * ML Super-Resolution
 *
 * In-browser neural upscaling via ONNX Runtime Web. Image pixels never
 * leave the machine; only the runtime and model weights are fetched
 * once and cached. Supports a built-in luminance model and custom RGB
 * .onnx models, processed as overlapping tiles to hide seams.
 */
...
function ensureOrtLoaded() {
    // Lazily injects ONNX Runtime Web (WebGPU bundle with WASM fallback)
...
async function cachedModelFetch(url, onStatus, signal, streamProgress = true) {
    // Fetches model bytes through the Cache API for one-time, offline-capable downloads
    if ('caches' in window) {
        try {
...
function tilePositions(extent, tile, step) {
    // Returns tile origins along one axis, the last tile flush against the far edge
    const positions = [];
    for (let p = 0; ; p += step) {
        if (p + tile >= extent) {
...
function padCanvasToMin(src, minSize) {
    // Pads a canvas to at least minSize by replicating the last row and column
...
export class SuperResolver {
    ...
    async loadFromUrl(url, label, filename, onStatus, expectedSha256 = null, signal = null) {
        // Downloads an RGB model (or loads from cache), verifying the pinned default's SHA-256
        const ort = await ensureOrtLoaded();
        onStatus?.('Connecting...');
    ...
    saveModelToDisk() {
        // Offers the downloaded model bytes as a file download for offline reuse
    ...
    async runTilePlanes(planes, n, ensemble) {
        // Runs the model on planar float input, optionally as an 8-variant self-ensemble
    ...
    async upscaleY(srcCanvas, onProgress, signal = null, opts = {}) {
        // Built-in path: 224x224 luminance tiles, 3x output, color from a bicubic upscale
    ...
    async probeScale() {
        // Determines an RGB model's scale factor and whether its input shape is fixed
        const ort = window.ort;
        const inName = this.session.inputNames[0];
        const outName = this.session.outputNames[0];
    ...
    async upscaleRGB(srcCanvas, onProgress, signal = null, opts = {}) {
        // Custom path: tiled NCHW float32 RGB inference in [0,1]
    ...
    stitchTile(outCtx, tileImage, tx, ty, tile, overlap, scale, PW, PH) {
        // Writes a processed tile into the output, discarding overlap margins
...
function smoothChroma(data, width, height, radius) {
    // Box-blurs only the Cb/Cr planes, leaving luminance untouched
...
export async function backProject(srcCanvas, upCanvas, iterations = 2, alpha = 0.75, onProgress, signal = null) {
    // Iterative back-projection: refines the upscaled canvas to match the source
    const W = srcCanvas.width, H = srcCanvas.height;
...
export function postprocessForText(canvas) {
    // Stroke-tuned finishing: local contrast then a small-radius unsharp mask
    const ctx = canvas.getContext('2d', { willReadFrequently: true });
    const img = ctx.getImageData(0, 0, canvas.width, canvas.height);
...
function transformPlane(plane, n, k, flip) {
    // Forward TTA transform: optional horizontal flip, then k clockwise 90-degree rotations
